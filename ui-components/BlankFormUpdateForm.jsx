...
export default function BlankFormUpdateForm(props) {
  const {
    id,
    blankForm,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...blankFormRecord };
    setEmail(cleanValues.email);
    setWebsiteIfOwned(cleanValues.websiteIfOwned);
  ...
  const [blankFormRecord, setBlankFormRecord] = React.useState(blankForm);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(BlankForm, id) : blankForm;
      setBlankFormRecord(record);
    };
    queryData();
  }, [id, blankForm]);
  React.useEffect(resetStateValues, [blankFormRecord]);
  const validations = {
    email: [],
    websiteIfOwned: [],
    phoneNumber: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          const original = await DataStore.query(BlankForm, id);
          await DataStore.save(
            BlankForm.copyOf(original, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "BlankFormUpdateForm")}
    >
      <TextField
        label="Email"
        isRequired={false}
